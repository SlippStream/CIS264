...
void getWeatherData(WeatherData& w, Months x) {
    do {
        cout << "Please enter the total rainfall for month " << x << ": ";
        cin >> w.totalRainfall;
    } while (w.totalRainfall < 0);

    do {
        cout << "Please enter the highest temperature for month " << x << ": ";
        cin >> w.highTemp;
    } while(w.highTemp < -100 || w.highTemp > 140);

    do {
        cout << "Please enter the lowest temperature for month " << x << ": ";
        cin >> w.lowTemp;
...
void displayWeatherData(WeatherData w[]) {
    ...
    cout << "Highest temperature of the year was " << high << " degrees in month " << (Months)hm+1 << ".\n";
    cout << "Lowest temperature of the year was " << low << " degrees in month " << (Months)lm+1 << ".\n";
...
int main() {
    ...
    for (int i = 0; i < NUM_MONTHS; i++) {
        getWeatherData(months[i], (Months)(i+1));
    }
